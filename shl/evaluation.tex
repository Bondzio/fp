...
\paragraph{For the statistical evaluation}
of the measurements we relied on the \textit{python} programming language~
...
Further we assume to have a scalar function $f(x_0,...,x_N) \rightarrow \mathbb{R}$, then the variance
...
in the evaluation is to make sure that the peaks which are used can be related
...
\paragraph{At this stage we had}
to decide with which peaks we would fit linearly; we chose
...
\end{wraptable}
It is possible to relate the other peaks we see in the cobalt probe with their respective energies.
...
it definitely does not make sense to use a $\chi^2$-test, so there is no way to check the fidelity.}
    \label{fig:fit_peaks}
\end{figure}
...
the channels to the delay, we used the following
...
\subsection{Calculating the half life}
\paragraph{In this section}
we will make use of the previous sections: Forwarding the channel to the time regime and
reducing the background from our main figure in order to evaluate the half life period.\\\\
The measurement of randomized concidences lasted \textbf{6080 seconds} = 1 hour 41 minutes and 20 seconds, while
the that of delayed coincidences lasted \textbf{52658 seconds} =  14 hours, 37 minutes and 38 seconds. 
...
\end{enumerate}
The least squares fit with \eqref{eq:expfit} results in 
...
At this point we have to note that the following actions did not change the result in any way. We will also
...
in slope, such that any influence of the background would not change the slope of the fitted curve 
significantly. The main background parameter is $a$, which is really small within the fit. 
\item The flipping in the time regime, fitting before scaling to nanoseconds or taking a different function (like
$a+b\cdot c^{-t}$ or a linear fit in the log-plot) does not result in any different result, as we expected.
\end{itemize}
The summary and interpretation will be given in the following conclusion.
...
errors whereas the blue line refers to the fit.
The green line indicates the curve with the literature value for $\lambda$ such 
that $N(t) = a + b\cdot e^{-\lambda_{\mathrm{lit}}\cdot t}$. It is clearly visible that this line does not agree with

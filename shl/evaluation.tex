...
\end{figure}
\section{conclusion}
The fitting of the energy spectra supplied reasonable results, such that the fitting gave rise to a linear fitting
of the time with respect to the channels, as was expected.
Unfortunatelly we were not able to reproduce the literature value of $T_{1/2, \mathrm{lit}}=98$ns, our procedure
yields the result
\begin{equation}
T_{1/2} =\left [11.9 \pm 0.5 \right ] ns .
\end{equation}
Since the data resamples in a exponential way and the fitting paramter seem to agree very good with the data,
we would like to emphasize the possibiliy of a technical problem which we were not able to resolve yet. There
are two possibilities with varying probability:
\begin{itemize}
\item We might have chosen the wrong peak and therfore have measured a different half life period of a different 
process. Since there were some few peaks this is not impossible but rather unprobable,  however, since there
is this great disparty between literatur and measurement we conclude that this is a possibility.
\item Between delay and channel is an element which we have no considered yet, but which changes the timecaling
 (slope) in such way (probably 4x - 5x) that our data is probable correct, but the amplification measured with the
TCA is not possible to use. We tried a trial run for different factors and realizied that even small changes
shift the energy values. We think this possibility is more probable than the former. 
\end{itemize}

...
\begin{figure}[htpb]
    ...
    \caption{Distribution of the Monte Carlo data with respect to the $\cos(\theta)$ and $\cos(\mathrm{thru})$. You can
    ...
    the classification process enormously, since the order of removing the t-channel is not crucial to the particle numbers.}
    \label{fig:cos_figs}
\end{figure}
...
\begin{table}[htpb]
    \centering
    \label{tab:eff1}
    \caption[caption]{Efficiency matrix with \textbf{physical intuition}\footnotemark.}
...
\paragraph{\textbf{Muon asymmetry:}}
\label{par:assymetry}

...
for sample_name in ["Ge"]:
    ...
    for i in range(2):
        ...
        """
        a_range = np.where((angle > -60) * (angle < -25))[0]
        print(len(a_range), angle[a_range][[0, -1]])

        a_range_lamp = np.where((angle_lamp > -60) * (angle_lamp < -25))[0]
        print(len(a_range_lamp), angle_lamp[a_range_lamp][[0, -1]])

        a_range_bg = np.where((angle_bg > -60) * (angle_bg < -25))[0]
        print(len(a_range_bg), angle_bg[a_range_bg][[0, -1]])
        """

        """
            if sample_name == "Si":
                for i in "12345":
                    file_in  = npy_dir + "band_gap_error_" + i
                    vernier_to_npy(file_in, file_out)
           """     
    """
    diff = angle[1:-1] - angle[:-2]
    diff_lamp = angle_lamp[1:-1] - angle_lamp[:-2]
    diff_bg = angle_bg[1:-1] - angle_bg[:-2]
    fig2, ax2 = plt.subplots(1, 1)
    ax2.hist(diff, bins=100)
    ax2.hist(diff_lamp, bins=100)
    ax2.hist(diff_bg, bins=100)
    #fig2.show()
"""

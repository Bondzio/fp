...
def poly(x, p0, p1, p2, p3, p4):
    x_p = x - 540 # move x value near the points to be fitted
...
def uc_str(c, max_digit=4):
    ...
    as floats with max max_digit (=4) error digits
    as exp else
    """
...
def rm_outliers(x, y, y_e, n=1):
    """
    Remove outliers identified by largest absolute value of 
    difference between point and its two neighbors.
    These are defects of the CCD
    """
    for j in range(1):
...
for i, suffix, label in enum(suffixes, labels):
    ...
    x, y, y_e = rm_outliers(x, y, y_e, 1)
    # Peak fit: Stokes 2,3, Anti-Stokes 2
    x_ranges = [[544, 553], [553, 560], [512, 517]]
    p0s = np.array([[551, 0.2, 10000, 0],[555, 0.2, 2000, 0], 
                    [514, 0.2, 200, 0]])
...
for i, suffix, label in enum(suffixes, labels):
    ...
    t, avg, x, y, y_e = get_ccd_rate(filename)
    x, y, y_e = rm_outliers(x, y, y_e, 2)
    # Peak fit: Stokes 1 - 4, Anti-Stokes 1, 2
    x_ranges = [[541, 548], [548, 552.5], [553, 560], [566, 572], 
                [519, 523], [512, 516], [509, 512]]
    p0s = np.array([[543, 0.2, 2000, 600], [551.5, 0.2, 2000, 600], 
                    [554, 0.4, 700, 600],  [569, 0.3, 700, 600], 
                    [522, 0.3, 1000, 100], [514, 0.3, 1000, 100], 
                    [511, 0.3, 100, 100]])
...
for i, suffix, label in enum(suffixes, labels):
    ...
    y = corr * y
    y_e = corr * y_e  
    x, y, y_e = rm_outliers(x, y, y_e, 5)
    # Peak fit: Stokes 1 - 3, Anti-Stokes 1, 2
    x_ranges = [[537, 543], [544, 552], [548, 560], [575, 585], 
                [520, 526], [514, 520], [507, 516]]
    p0s = np.array([[541, 0.2, 1000, 0], [546, 0.2, 2000, 0],
                    [555, 0.2, 400, 0], [579, 0.2, 60, 0], 
                    [523, 0.2, 200, 0], [519, 0.2, 200, 0], 
                    [511, 0.2, 30, 0]])
    for x_range, p0 in zip(x_ranges, p0s):
        x_fit, fit_peak, c1 = bw_fit(x, y_e, x_range, p0, fit=True)
        x0s.append(uc.ufloat(c1[0].n, c1[1].n))
...
for i, conc_i, label in enum(concentration, labels):
    ...
    x_range_poly = [550,  610] # reduce range for polyfit
    ...
    # Peak fit: Stokes 2,3, Anti-Stokes 2
    x_ranges = [[554, 561], [569, 573], [573, 580]]
    p0s = np.array([[558, 0.2, 100, 0], [571, 0.2, 50, 0], 
                    [577, 0.2, 50, 0]])
    for j, x_range, p0 in enum(x_ranges, p0s):
...
for i, I_laser, label in enum(I_laser_all, labels):
    ...
    y = corr * y
    y_e = corr * y_e
    x_ranges = [[545, 549], [510, 519]]
    p0s = np.array([[545.5, 0.2, 20000, 3500], [518, 0.2, 5000, 200]])
    for j, x_range, p0 in enum(x_ranges, p0s):
        x_fit, fit_peak, c1 = bw_fit(x, y_e, x_range, p0, fit=True)
        x0s.append(c1[0])
        As.append(c1[2])

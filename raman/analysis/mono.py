...
x_ranges = [[402, 406], [406, 410], [430, 440], 
            [540, 550], [570, 578], [578, 584]]
labels = ['404.7 nm', '407.8 nm', '435.8 nm', 
    '546.1 nm', '577.1 nm', '579.1 nm']
x0s = []
...
for i, width in enumerate(widths):
    ...
    x = np.load("npy/" + filename + "_lamb.npy")[1:] / 10
    y0 = np.load("npy/" + filename + "_count.npy")[1:]
...
y90 = np.load("npy/"+filename+"_count.npy")[1:]

# CS2
filename = 'mono_cs2_04' # 04: Stokes at 551 nm
x = np.load("npy/"+filename+"_lamb.npy")[1:] / 10
y = np.load("npy/"+filename+"_count.npy")[1:]
x_ranges = [[549, 554]]
labels = ['551.3 nm']
x0s = []
for i, x_range, label in zip(range(len(x_ranges)), x_ranges, labels):
...
for i, x0, dnu_cm, lit in enum(x0s, lamb_to_cm(x0s), lits):

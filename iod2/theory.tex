...
we can write down the energy eigenvalues in terms of the two 
quantum numbers
$\nu$ and $J$:
\begin{equation}
    E = E_{\nu,J}
...
so we look at two simplified extrem cases.

\paragraph{The rigid Rotator} is based on the approximation to
fix the distance between the two nuclei ($r =$const.) such that 
...
arising from rotation, which is opportune, looking at the length scale of
the former calculated eigenvalues.
